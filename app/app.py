...
users = [["mark.bailey", "Mark Bailey"], ["john.williams", "John Williams"]]
...
def index():
    ...
    for c in clinicalRequestTypes:
        clinicalRequestTypesHTML += preparedFormElements.wrapHTML(
            "",
            f"""<div>
...
def clinicalRequesting():
    ...
    placeholders.extend(
        docxPtr.get_placeholders(lastSelectedLocation, requestsChecked)
    )
...
def clinicalRequestSubmit():
    ...
    if request.method == "POST":
        ...
        preparedFormElements = formElements()
        placeHoldersUpdated = preparedFormElements.extractResults(
            session["placeholders"], request
        )
        ...
        for index, item in enumerate(placeHoldersUpdated):
            ...
            if item[0].lower() == "signature|picture":
                placeHoldersUpdated[index] = [
                    "signature|picture",
        ...
        print("***")
        print(placeHoldersUpdated)
        print("***")

        for r in session["requestsChecked"]:
            PDFPath = docxPtr.create(
